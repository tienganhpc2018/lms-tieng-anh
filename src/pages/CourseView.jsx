...
export default function CourseView() {
  ...
  // Cập nhật Sửa Bài học (Làm sạch 100% Tiêu đề - Giữ nguyên tên ngắn gọn của Thầy Hải)
  const handleSaveEditActivity = async (e) => {
    e.preventDefault();
    if (!editingAct) return;

    let formattedTitle = getCleanTitle(editTitle);
    let targetDbType = getValidDbType(editType);
    ...
    try {
      await supabase
        .from('activities')
        .update({ title: formattedTitle, type: targetDbType, content: editContent, settings: updatedSettings })
        .eq('id', editingAct.id);
    } catch (err) {}
  ...
  const handleCreateActivity = async (e) => {
    ...
    try {
      ...
      // Tự động bảo vệ dbType luôn hợp lệ với Supabase Postgres Check Constraint ('quiz', 'assignment', 'whiteboard', 'iframe', 'page', 'video')
      const dbType = getValidDbType(newActType);

      const formattedTitle = getCleanTitle(newActTitle);
        ...
        .insert([
          {
            ...
            settings: { customType: newActType },
                      ...
                      <div
                        ...
                        className={`p-4 rounded-2xl border transition flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 group ${
                          act.is_hidden
                            ...
                            : isWhiteboardAct(act)
                            ? 'bg-amber-50 hover:bg-amber-100 border-amber-300' : isDictationAct(act)
                            ? 'bg-purple-50 hover:bg-purple-100 border-purple-300' : isAudioRecordAct(act)
                          ...
                          <div className="space-y-0.5">
                            ...
                            <div className="flex items-center space-x-3 text-[11px] text-slate-500 font-medium flex-wrap gap-y-1">
                              <span className="font-extrabold text-slate-600 uppercase">
                                {isWorksheetAct(act) ? '📑 Worksheet (Tập Bài Tập Tổng Hợp H5P)' : isWhiteboardAct(act) ? '🎨 Whiteboard Bảng Tương Tác' : isDictationAct(act) ? '🎧 Dictation (Nghe Chép Chính Tả H5P)' : isAudioRecordAct(act) ? '🎙️ Audio Recorder Bảng Luyện Nói' : isInteractiveVideoAct(act) ? '🎥 Interactive Video (Video Tương Tác H5P)' : act.type}
                              </span>
                          ...
                          {userIsTeacher && (
                            <div className="flex items-center space-x-1 flex-wrap gap-y-1" onClick={(e) => e.stopPropagation()}>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setEditingAct(act);
                                  setEditTitle(getCleanTitle(act.title));
                                  setEditType(act.settings?.customType || (isWhiteboardAct(act) ? 'whiteboard' : isDictationAct(act) ? 'dictation' : isAudioRecordAct(act) ? 'audio_record' : isInteractiveVideoAct(act) ? 'video' : act.type || 'quiz'));
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 animate-scale-up">
            ...
            <form onSubmit={handleCreateActivity} className="p-6 space-y-4">
              ...
              <div>
                ...
                >
                  <option value="vocabulary">📚 Vocabulary (Từ Vựng Tương Tác & Từ Điển Glossary H5P)</option>
                  <option value="worksheet">📑 Worksheet (Tập Bài Tập Tổng Hợp Multi-Task H5P)</option>
                  <option value="dictation">🎧 Dictation (Bài Tập Nghe Chép Chính Tả H5P)</option>
