...
export default function QuizEngine({ activity, activityId, onComplete }) {
  ...
  useEffect(() => {
    if (submitted) {
      try {
        ...
        console.error('Auto pause audio effect error:', audioErr);
      }
    }
  }, [submitted]);
    ...
    async function fetchQuestions() {
      ...
      try {
        ...
        if (error) {
          ...
          // KIỂM TRA & NẠP BÀI LÀM CỦA HỌC SINH (CHỈ NẠP KHI CÓ THAM SỐ XEM LẠI BÀI VÀO URL: submissionId HOẶC review=true)
          const searchParams = new URLSearchParams(window.location.search);
          const urlSubmissionId = searchParams.get('submissionId');
          const urlStudentId = searchParams.get('studentId');
          const isExplicitReview = searchParams.get('mode') === 'review' || searchParams.get('review') === 'true' || !!urlSubmissionId || !!urlStudentId;
  ...
  const handleSubmitQuiz = async (isAutoSubmit = false) => {
    ...
    // DỪNG PHÁT TOÀN BỘ BÀI NGHE AUDIO NGAY LẬP TỨC KHI HỌC SINH BẤM NỘP BÀI
    try {
      const allAudios = document.querySelectorAll('audio');
      allAudios.forEach((audio) => {
        audio.pause();
      ...
      console.error('Auto pause audio error in handleSubmit:', audioErr);
    }
