import { X, FileText, HelpCircle, Video, Package, Upload, Link, BookOpen, Layers, Headphones } from 'lucide-react';
import { uploadLMSFile } from '../../lib/supabase';
import RichTextEditor from './RichTextEditor';
import { getValidDbType } from '../../utils/activityTypeHelpers';

const ACTIVITY_TYPES = [
  { id: 'vocabulary', name: 'Vocabulary (Từ Vựng Glossary H5P)', icon: BookOpen, desc: 'Từ điển tương tác Glossary H5P phát âm, hình ảnh, câu mẫu & audio', color: 'bg-amber-50 text-amber-600' },
  { id: 'worksheet', name: 'Worksheet (Tập Bài Tập Tổng Hợp H5P)', icon: FileText, desc: 'Tập bài tập tổng hợp 7 dạng bài (Fill gaps, Drag words, Listening, Unscramble, Rewrite, MCQ)', color: 'bg-indigo-50 text-indigo-600' },
  ...
  { id: 'dictation', name: 'Nghe Chép Chính Tả (Dictation)', icon: Headphones, desc: 'Nghe audio tốc độ thường / chậm và chép chính tả H5P', color: 'bg-purple-50 text-purple-600' },
...
export default function ActivityModal({ isOpen, onClose, onAddActivity, sectionId }) {
  ...
  // Settings riêng cho Interactive Video, Assignment hoặc Vocabulary Link
  const [videoUrl, setVideoUrl] = useState('');
  const [deadline, setDeadline] = useState('');
  const [lessonSection, setLessonSection] = useState('auto');
  ...
  const handleSubmit = (e) => {
    ...
    const dbType = getValidDbType(selectedType);
    const settings = {
          ...
          {selectedType === 'vocabulary' && (
            <div className="p-3.5 bg-amber-50 rounded-xl border border-amber-300 space-y-2">
              <label className="block text-xs font-bold text-amber-900 uppercase tracking-wider">
                🔗 Liên Kết Với Tiết Học (Lesson Section Trong Unit)
              </label>
              <select
                value={lessonSection}
                onChange={(e) => setLessonSection(e.target.value)}
                className="w-full px-3 py-2 border border-amber-300 rounded-lg text-xs font-bold text-amber-950 bg-white focus:ring-2 focus:ring-amber-500"
              >
                <option value="auto">✨ Tự Động Theo Tên Bài (Auto Detect từ tiêu đề bài học)</option>
                <option value="All">🌐 Tất Cả (All Lessons)</option>
                <option value="GETTING STARTED">🚀 1. GETTING STARTED</option>
                <option value="A CLOSER LOOK 1">📖 2. A CLOSER LOOK 1</option>
                <option value="A CLOSER LOOK 2">⚡ 3. A CLOSER LOOK 2</option>
                <option value="COMMUNICATION">💬 4. COMMUNICATION</option>
                <option value="SKILLS 1">🎧 5. SKILLS 1</option>
                <option value="SKILLS 2">✍️ 6. SKILLS 2</option>
                <option value="LOOKING BACK">🏆 7. LOOKING BACK</option>
                <option value="PROJECT">🎨 8. PROJECT</option>
              </select>
              <p className="text-[11px] text-amber-800 font-medium">
                💡 Khi nhấp vào bài học này, từ điển sẽ tự động lọc đúng danh mục từ vựng của tiết học đó (Ví dụ: Getting Started, A Closer Look 1...).
              </p>
            </div>
          )}
