...
const FIREWORK_WORD_COLORS = [
  { bg: '#059669', stroke: '#6ee7b7', text: '#ffffff', glow: 'rgba(5, 150, 105, 0.85)', name: 'Bright Emerald' },
  { bg: '#e11d48', stroke: '#fda4af', text: '#ffffff', glow: 'rgba(225, 29, 72, 0.85)', name: 'Neon Rose' },
  { bg: '#d97706', stroke: '#fde047', text: '#ffffff', glow: 'rgba(217, 119, 6, 0.85)', name: 'Sun Amber' },
  { bg: '#0284c7', stroke: '#7dd3fc', text: '#ffffff', glow: 'rgba(2, 132, 199, 0.85)', name: 'Electric Sky' },
  { bg: '#7c3aed', stroke: '#d8b4fe', text: '#ffffff', glow: 'rgba(124, 58, 237, 0.85)', name: 'Vivid Purple' },
  { bg: '#ea580c', stroke: '#fdba74', text: '#ffffff', glow: 'rgba(234, 88, 12, 0.85)', name: 'Flame Orange' },
  { bg: '#db2777', stroke: '#fbcfe8', text: '#ffffff', glow: 'rgba(219, 39, 119, 0.85)', name: 'Hot Pink' },
  { bg: '#16a34a', stroke: '#86efac', text: '#ffffff', glow: 'rgba(22, 163, 74, 0.85)', name: 'Vibrant Green' },
  { bg: '#0891b2', stroke: '#67e8f9', text: '#ffffff', glow: 'rgba(8, 145, 178, 0.85)', name: 'Radiant Cyan' },
  { bg: '#4f46e5', stroke: '#a5b4fc', text: '#ffffff', glow: 'rgba(79, 70, 229, 0.85)', name: 'Royal Indigo' },
  { bg: '#c026d3', stroke: '#f0abfc', text: '#ffffff', glow: 'rgba(192, 38, 211, 0.85)', name: 'Electric Fuchsia' },
  { bg: '#0d9488', stroke: '#5eead4', text: '#ffffff', glow: 'rgba(13, 148, 136, 0.85)', name: 'Aqua Teal' },
  { bg: '#ca8a04', stroke: '#fef08a', text: '#ffffff', glow: 'rgba(202, 138, 4, 0.85)', name: 'Goldenrod' },
  { bg: '#2563eb', stroke: '#93c5fd', text: '#ffffff', glow: 'rgba(37, 99, 235, 0.85)', name: 'Cobalt Spark' },
  { bg: '#f43f5e', stroke: '#fecdd3', text: '#ffffff', glow: 'rgba(244, 63, 94, 0.85)', name: 'Coral Fire' },
  { bg: '#8b5cf6', stroke: '#c4b5fd', text: '#ffffff', glow: 'rgba(139, 92, 246, 0.85)', name: 'Bright Violet' },
];
...
export default function VocabularyEngine({ activity, isTeacher = false, onSaveActivity }) {
      ...
      audio.ontimeupdate = () => {
        ...
        for (let i = 0; i < activeTabObj.lines.length; i++) {
          ...
          if (l.startTime !== undefined && l.endTime !== undefined) {
            if (cur >= l.startTime && cur < l.endTime) {
              matchedIdx = i;
              break;
            }
          }
  ...
  const handleStartRolePlay = () => {
    ...
    setIsPlayingFullDialogue(true);
    playRolePlayStep(0, activeTabObj.lines);
  };

  const playRolePlayStep = (index, lines) => {
    ...
    const speakerName = (currentLine.speaker || '').trim().toLowerCase();
    const userChar = (userSelectedCharacter || '').trim().toLowerCase();
    const isUserTurn = speakerName === userChar;

    setPlayingDialogueLineIndex(index);
    setRolePlayStepIndex(index);

    if (isUserTurn) {
      setIsWaitingForUserRead(true);
      playSuccessSound();
    } else {
      setIsWaitingForUserRead(false);
      speakDialogueLine(currentLine, () => {
        const nextIdx = index + 1;
        playRolePlayStep(nextIdx, lines);
      });
    }
  };
  ...
  const handleStartRecordingLine = (index, lineObj) => {
    if (typeof window === 'undefined') return;
    ...
    if (!SpeechRecognition) {
      ...
      setIsDialogueLineRecording(true);
      setTimeout(() => {
        setIsDialogueLineRecording(false);
        setRecordingLineIndex(null);
        const simulatedScore = Math.floor(Math.random() * 14) + 85;
        setLineScores(prev => ({ ...prev, [index]: simulatedScore }));
        playSuccessSound();
      }, 2000);
      return;
    }
    ...
    try {
      ...
      recognition.onresult = (event) => {
        ...
        const targetText = lineObj.text.toLowerCase().trim();
        ...
        targetWords.forEach(w => {
          ...
          if (cleanW && userWords.some(uw => uw.replace(/[^\w]/g, '') === cleanW)) {
            matchedCount++;
          }
        });

        let rawScore = Math.round((matchedCount / Math.max(targetWords.length, 1)) * 100);
        if (userText.length > 3 && rawScore < 60) rawScore = Math.min(80, rawScore + 40);
        if (rawScore > 100) rawScore = 100;

        setLineScores(prev => ({ ...prev, [index]: rawScore }));
        setIsDialogueLineRecording(false);
        setRecordingLineIndex(null);
        playSuccessSound();
      };
      ...
      recognition.onerror = () => {
        ...
        setLineScores(prev => ({ ...prev, [index]: fallbackScore }));
        playSuccessSound();
      };
  ...
  const handlePrintDialogueLesson = () => {
...
// REMOVED AUTO-REPAIR EFFECT (NEVER OVERWRITE TEACHER VOCABULARY LIST WITH GRADE 7 PRESET)
  ...
  const renderHighlightedStoryText = (text, list) => {
    ...
    const meaningList = list
      .slice(0, 10)
      .map((i) => {
        if (!i || !i.meaning) return '';
        const raw = String(i.meaning).split('/')[0].split(',')[0].toLowerCase().trim();
        return raw;
      })
      .filter(Boolean);
...
`;
  ...
  // TẢI BẢN IN TRONG SUỐT CHO HỌC SINH TÔ MÀU BẰNG BÚT MÀU TRÊN LỚP (THEO YÊU CẦU THẦY HẢI)
  const handleDownloadTransparentColoringSheet = () => {
    if (!wordSearchData || !wordSearchData.grid) {
      alert("Chưa có dữ liệu ô chữ để tải về!");
      return;
    }

    try {
      const isCircle = wordSearchShape === 'circle';
      const canvas = document.createElement('canvas');
      const width = 1200;
      const height = isCircle ? 1450 : 1350;
      canvas.width = width;
      canvas.height = height;
      const ctx = canvas.getContext('2d');
      if (!ctx) return;

      // NỀN TRONG SUỐT HOÀN TOÀN (TRANSPARENT BACKGROUND)
      ctx.clearRect(0, 0, width, height);

      // TIÊU ĐỀ BÀI TẬP IN ĐEN TRẮNG TRONG SUỐT CHO HS TÔ MÀU
      ...
      ctx.fillText(
        isCircle
          ? '🎆 VÒNG XOAY TÌM TỪ PHÁO HOA LUNG LINH'
          : '🟦 BẢNG TÌM TỪ TIẾNG ANH (FIND THE WORD)',
        width / 2,
        35
      );
      ...
      ctx.fillText('BÀI TẬP TÌM TỪ KHÓA & TÔ MÀU SÁNG TẠO (COLORING WORKSHEET)', width / 2, 85);
      ...
      if (isCircle) {
        ...
        const canvasFontSizes = [28, 32, 36, 40, 44];

        // Vành ngoài cùng tròn
        ctx.beginPath();
        ctx.arc(cx, cy, 465, 0, 2 * Math.PI);
        ctx.strokeStyle = '#0f172a';
        ctx.lineWidth = 4;
        ctx.stroke();

        wordSearchData.grid.forEach((row, r) => {
          const rIn = ringRadii[r]?.in || 105;
          const rOut = ringRadii[r]?.out || 175;
          const rMid = (rIn + rOut) / 2;
          const fSize = canvasFontSizes[r] || 32;

          row.forEach((char, c) => {
            const startAngleDeg = c * sectorAngle;
            const endAngleDeg = (c + 1) * sectorAngle;
            const startRad = ((startAngleDeg - 90) * Math.PI) / 180;
            const endRad = ((endAngleDeg - 90) * Math.PI) / 180;

            // Vẽ viền nan quạt nét đen rõ
            ctx.beginPath();
            ...
            // Vẽ chữ cái đen đậm nét
            const midAngleDeg = startAngleDeg + sectorAngle / 2;
        ...
        const gridSize = wordSearchData.grid.length;
        const cellSize = 65;
      ...
      // HỘP DANH SÁCH TỪ VỰNG PHÍA DƯỚI ĐỂ HS CHECK & TÔ MÀU
      const boxY = isCircle ? 1160 : 880;
      ctx.strokeStyle = '#334155';
      ctx.lineWidth = 2;
      ctx.setLineDash([8, 6]);
      ctx.strokeRect(80, boxY, width - 160, isCircle ? 230 : 250);
      ctx.setLineDash([]);
      ...
      ctx.textAlign = 'left';
      ctx.textBaseline = 'top';
      ctx.fillText(`📋 DANH SÁCH TỪ VỰNG CẦN TÌM VÀ TÔ MÀU (${wordSearchData.placedWords.length} TỪ):`, 110, boxY + 20);

      ctx.font = '16px sans-serif';
      ctx.fillStyle = '#475569';
      ctx.fillText('(Học sinh dùng bút màu khác nhau tô kín các chữ cái tìm được, sau đó đánh dấu ✓ vào ô trống)', 110, boxY + 50);

      const words = wordSearchData.placedWords || [];
      const cols = 3;
      const colWidth = 330;
      words.forEach((pw, idx) => {
        const c = idx % cols;
        const row = Math.floor(idx / cols);
        const wx = 110 + c * colWidth;
        const wy = boxY + 90 + row * 40;

        // Ô vuông checkbox cho HS tích
        ctx.strokeStyle = '#0f172a';
        ctx.lineWidth = 2;
        ...
        ctx.font = 'bold 20px sans-serif';
        ctx.fillStyle = '#0f172a';
        ctx.fillText(`${idx + 1}. ${pw.word}`, wx + 36, wy + 2);
      });

      // TẢI FILE PNG TRONG SUỐT VỀ MÁY
      const dataUrl = canvas.toDataURL('image/png');
      const a = document.createElement('a');
      a.href = dataUrl;
      a.download = isCircle
        ? `Vong-Xoay-Phao-Hoa-To-Mau-Unit1.png`
        : `O-Chu-Tim-Tu-To-Mau-Unit1.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      playSuccessSound();
      alert("🎉 Đã tải file ảnh ô chữ trong suốt thành công! Thầy có thể in trực tiếp ra giấy A4 hoặc chèn vào phiếu bài tập cho học sinh tô màu nhé.");
    } catch (err) {
      console.error('Download coloring sheet error:', err);
      alert("Có lỗi khi tạo file ảnh tải về: " + err.message);
    }
  };
      ...
      {activeTab === 'dialogue_lesson' && (
        <div className="bg-white rounded-3xl p-4 sm:p-6 border-4 border-purple-400 shadow-2xl space-y-5 animate-fade-in text-slate-900 my-4 print:p-0 print:border-none print:shadow-none relative">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-purple-100 pb-4 print:hidden">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {/* NÚT TỐC ĐỘ ĐỌC (0.8x, 1.0x, 1.2x) */}
              <div className="flex items-center bg-purple-50 p-1 rounded-2xl border border-purple-200 shadow-2xs">
                <span className="text-[10px] font-black text-purple-900 px-1.5 flex items-center space-x-1">
                  <span>⏩ Tốc độ:</span>
                </span>
                {[
                  { speed: 0.8, label: '0.8x (Chậm SGK)' },
                  { speed: 1.0, label: '1.0x (Chuẩn)' },
                  { speed: 1.2, label: '1.2x (Nhanh)' }
                ].map((sObj) => (
                  <button
                    key={sObj.speed}
                    type="button"
                    onClick={() => setDialogueSpeed(sObj.speed)}
                    className={`px-2 py-1 rounded-xl text-[11px] font-black transition cursor-pointer ${
                      dialogueSpeed === sObj.speed
              ...
              <button
                type="button"
                onClick={() => setShowDialogueVietnamese(!showDialogueVietnamese)}
                className={`px-3 py-2 rounded-2xl font-extrabold text-xs transition cursor-pointer flex items-center space-x-1.5 border shadow-2xs ${
                  showDialogueVietnamese
              ...
              <button
                type="button"
                onClick={handlePrintDialogueLesson}
                className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs rounded-2xl shadow-sm transition cursor-pointer flex items-center space-x-1 border border-indigo-400"
                title="In bài học hoặc xuất file PDF chuẩn SGK"
              >
                ...
                <div className="flex items-center space-x-2">
                  {!isPlayingFullDialogue ? (
                    <button
                      ...
                      className="px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-extrabold text-xs sm:text-sm rounded-2xl shadow-md transition transform hover:scale-105 cursor-pointer flex items-center space-x-1.5 border border-purple-400"
                    >
                      <span>🚀 Bắt Đầu Đóng Vai</span>
                    </button>
                  ) : (
                    <button
                      type="button"
              ...
              <div className="flex items-center space-x-2 bg-white px-3 py-1.5 rounded-xl border border-purple-200">
                ...
                {['School Counsellor', 'Ann', 'Mi', 'Nick', 'Mai'].map((char) => (
                  <button
                    key={char}
                    type="button"
                    onClick={() => setUserSelectedCharacter(char)}
                    className={`px-2.5 py-1 rounded-lg text-xs font-black transition cursor-pointer ${
                      userSelectedCharacter.toLowerCase() === char.toLowerCase()
          ...
          {/* ROLE-PLAY INTERACTIVE STATUS BANNER */}
          {isRolePlayMode && isWaitingForUserRead && (
            <div className="bg-gradient-to-r from-amber-400 via-amber-300 to-amber-400 p-4 rounded-2xl border-2 border-amber-500 shadow-md flex flex-col sm:flex-row items-center justify-between gap-3 animate-pulse print:hidden">
              <div className="flex items-center space-x-2 text-slate-950 font-black text-sm">
                <span className="text-2xl">👉</span>
                <div>
                  <p className="uppercase tracking-wide text-xs text-amber-950">ĐẾN LƯỢT HỌC SINH ĐÓNG VAI ({userSelectedCharacter}):</p>
                  <p className="text-sm sm:text-base">Hãy đọc to câu thoại màu vàng/xanh lá bên dưới, sau đó bấm nút để AI phản hồi tiếp!</p>
                </div>
              </div>
              <button
                type="button"
                onClick={handleUserFinishedReadingLine}
                className="px-5 py-2.5 bg-slate-950 hover:bg-slate-900 text-amber-300 font-extrabold text-xs sm:text-sm rounded-xl shadow-lg transition transform hover:scale-105 cursor-pointer shrink-0 border border-amber-400 flex items-center space-x-1.5"
              >
                <span>🎤 Tôi Đã Đọc Xong (AI Đọc Tiếp) ➔</span>
              </button>
            </div>
          )}
                  ...
                  <div className="sm:col-span-3 bg-emerald-50 p-3 rounded-2xl border-2 border-emerald-300 space-y-2">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span className="text-xs font-black text-emerald-950 flex items-center space-x-1.5">
                        <span>🎧</span>
                        <span>Trình Phát Audio Gốc SGK:</span>
                      </span>
                      <audio
                        ref={modalAudioPreviewRef}
                        controls
                        src={editingDialogueObj.audioUrl}
                        className="h-7 max-w-xs w-full"
                      />
                    </div>
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-emerald-200">
                      <div className="flex items-center space-x-2">
                        <span className="text-xs font-bold text-emerald-800">⏱️ Bỏ qua nhạc dạo/intro (giây):</span>
                        <input
                          type="number"
                          step="0.5"
                          min="0"
                      ...
                      <button
                        ...
                        className="px-3 py-1 bg-emerald-700 hover:bg-emerald-800 text-white font-black text-xs rounded-xl shadow-xs transition cursor-pointer flex items-center space-x-1"
                        title="Tự động tính toán mốc giây đọc cho từng câu theo độ dài lời thoại"
                      >
                        <span>⚡ Tự Động Khớp Mốc Giây Từng Câu</span>
                      </button>
                    </div>
                  </div>
                )}
                      ...
                      >
                        <div className="space-y-1 grow">
                          ...
                          {(showDialogueVietnamese || isRevealedVi) ? (
                            ...
                            <button
                              type="button"
                              onClick={() => setRevealedTranslationIndexes(prev => ({ ...prev, [idx]: true }))}
                              className="text-[11px] text-purple-700 font-bold hover:underline italic pl-1 cursor-pointer flex items-center space-x-1 print:hidden"
                            >
                              <span>👁️ [Nhấp để xem dịch Tiếng Việt]</span>
                            </button>
                          )}
                        </div>
        ...
        <div className="bg-amber-950/80 rounded-2xl p-4 sm:p-6 border-2 border-amber-700 text-white space-y-4 animate-fade-in">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-amber-800 pb-3">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs font-extrabold">
              ...
              {/* NÚT TẢI BẢN TRONG SUỐT CHO HS TÔ MÀU THEO YÊU CẦU THẦY HẢI */}
              <button
                type="button"
                onClick={handleDownloadTransparentColoringSheet}
                className="px-3.5 py-1.5 bg-gradient-to-r from-teal-400 via-emerald-400 to-teal-500 hover:from-teal-300 hover:to-emerald-400 text-slate-950 font-black rounded-xl transition cursor-pointer flex items-center space-x-1.5 shadow-md border border-teal-200 transform hover:scale-105"
                title="Tải file ảnh ô chữ nền trong suốt độ nét cao để in phiếu bài tập cho học sinh tự tô màu bằng bút màu trên lớp!"
              >
                <Download className="w-4 h-4 text-slate-950" />
                <span>📥 Tải File Tô Màu (Trong Suốt)</span>
              </button>
      ...
      {isContextStudioOpen && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-950/85 backdrop-blur-md p-3 sm:p-5 animate-fade-in overflow-y-auto">
          <div className="bg-white rounded-3xl p-5 sm:p-6 max-w-2xl w-full border-4 border-pink-500 shadow-2xl space-y-4 text-slate-900 my-auto">
            ...
            {/* HEADER MODAL VỚI SELECTOR CHỌN UNIT */}
            <div className="flex flex-wrap items-center justify-between border-b border-slate-200 pb-3 gap-2">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-pink-700 font-bold">
                    Soạn ngữ cảnh SGK riêng cho từng Tiết học & từng Unit (Dán ảnh Ctrl+V hoặc nhập văn bản)
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                {/* SELECTOR CHỌN UNIT */}
                <select
                  value={activeStudioUnit}
                  onChange={(e) => setActiveStudioUnit(e.target.value)}
                  className="px-3 py-1.5 bg-pink-100 border border-pink-300 rounded-xl text-pink-950 font-black text-xs cursor-pointer focus:outline-none"
                >
                  {['Unit 1', 'Unit 2', 'Unit 3', 'Unit 4', 'Unit 5', 'Unit 6', 'Unit 7', 'Unit 8', 'Unit 9', 'Unit 10', 'Unit 11', 'Unit 12'].map((u) => (
            ...
            {/* THANH TAB CHỌN NHANH 8 TIẾT HỌC (QUICK 8-LESSON TAB BAR) */}
            <div className="space-y-1.5">
              <label className="block text-[11px] font-black text-slate-700 uppercase tracking-wide">
                📑 CHỌN TIẾT HỌC ĐỂ SOẠN NGỮ CẢNH ({activeStudioUnit}):
              </label>
              <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 scrollbar-thin">
                {[
                  ...
                  const isSelected = activeStudioSection === secName;
                  const hasData = !!getContextForLesson(activeStudioUnit, secName);
                  return (
                    <button
                      key={secName}
                      type="button"
                      onClick={() => setActiveStudioSection(secName)}
                      className={`px-3 py-1.5 rounded-xl font-extrabold text-[11px] whitespace-nowrap transition cursor-pointer flex items-center space-x-1 border shadow-2xs ${
                        isSelected
                    ...
                    >
                      ...
                      {hasData && <CheckCircle className="w-3.5 h-3.5 text-emerald-500 fill-emerald-100" />}
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-3.5 text-xs font-bold text-slate-800">
              ...
              <div className="bg-gradient-to-br from-pink-50 to-purple-50 p-4 rounded-2xl border-2 border-dashed border-pink-400 space-y-3 relative shadow-inner">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <label className="text-pink-950 font-black text-xs uppercase tracking-wide flex items-center space-x-1.5">
                    ...
                    <span>🖼️ DÁN / NẠP ẢNH TRANG SGK CHO [{activeStudioUnit} - {activeStudioSection}]:</span>
                  </label>
                ...
                <div
                  ...
                  className="bg-white/90 p-3.5 rounded-xl border-2 border-dashed border-pink-300 text-center space-y-1.5 cursor-pointer hover:bg-pink-100/50 transition focus:outline-none focus:ring-2 focus:ring-pink-500 shadow-xs"
                >
                  ...
                  <p className="text-[11px] text-pink-900/80 font-bold">
                    Hoặc chọn file ảnh chụp bên dưới ➔ AI sẽ tự động đọc và điền dữ liệu 3 ô cho tiết [{activeStudioSection}]!
                  </p>
                </div>
                ...
                {isScanningSgkImage && (
                  <p className="text-pink-700 font-black animate-pulse flex items-center space-x-1.5 pt-1">
                    ...
                    <span>Gemini Vision AI đang đọc ảnh SGK & tự động điền nhân vật, cốt truyện cho tiết [{activeStudioUnit} - {activeStudioSection}]...</span>
                  </p>
                )}

                {contextImagePreview && (
                  <div className="mt-2 relative max-h-44 overflow-hidden rounded-xl border-2 border-pink-400 shadow-md group">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <label className="block text-slate-900 font-black">
                    👥 TÊN CÁC NHÂN VẬT CHÍNH SGK ([{activeStudioUnit} - {activeStudioSection}]):
                  </label>
                ...
                <input
                  ...
                  placeholder="Để trống hoặc gõ tên nhân vật (Ví dụ: Mark, Lan...)"
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <label className="block text-slate-900 font-black">
                    📝 CỐT TRUYỆN GỐC & Ý CHÍNH CỦA TIẾT [{activeStudioSection}]:
                  </label>
                  <span className="text-[10px] bg-emerald-100 text-emerald-800 font-mono px-1.5 py-0.5 rounded font-black">
                    Tùy chỉnh / AI Auto-Fill ✓
                  </span>
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <label className="block text-slate-900 font-black">
                    ⚡ CẤU TRÚC NGỮ PHÁP / SPEAKING CHUẨN TIẾT [{activeStudioSection}]:
                  </label>
                  <span className="text-[10px] bg-emerald-100 text-emerald-800 font-mono px-1.5 py-0.5 rounded font-black">
                    Tùy chỉnh / AI Auto-Fill ✓
                  </span>
                ...
                <input
                  ...
                  placeholder="Ví dụ: Past Simple & Present Perfect..."
            ...
            <div className="pt-3 border-t border-slate-200 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  <span>💾 Lưu Ngữ Cảnh Tiết [{activeStudioSection}]</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 max-w-2xl w-full border-4 border-purple-500 shadow-2xl space-y-4 text-slate-900 my-auto max-h-[85vh] flex flex-col">
            ...
            {generatingStory ? (
              ...
              <div className="space-y-4 overflow-y-auto pr-1 grow">
                {/* THANH CHỌN 5 VERSION TRUYỆN DỄ DÀNG CHO HỌC SINH (V260) */}
                <div className="flex flex-wrap items-center gap-1.5 mb-2 bg-purple-100/70 p-2 rounded-2xl border border-purple-200 shadow-xs">
                  <span className="text-xs font-black text-purple-900 px-1">Chuyển Nhanh 5 Version Truyện:</span>
                  {[1, 2, 3, 4, 5].map((vNum) => (
                    <button
                      key={vNum}
                      type="button"
                      ...
                      className={`px-2.5 py-1 rounded-xl text-xs font-black transition cursor-pointer flex items-center space-x-1 shadow-xs ${
                        ((storySeed % 5) + 5) % 5 === vNum - 1
                          ? 'bg-purple-700 text-white ring-2 ring-purple-400 scale-105 shadow-sm'
                          : 'bg-white text-purple-950 hover:bg-purple-200 border border-purple-300'
                      }`}
                    >
