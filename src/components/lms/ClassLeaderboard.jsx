...
export default function ClassLeaderboard({ submissions = [], activityTitle, onSelectSub }) {
    ...
    <div className="bg-gradient-to-br from-slate-900 to-indigo-950 text-white p-5 rounded-3xl border border-indigo-800 shadow-xl space-y-4">
      <div className="flex justify-between items-center border-b border-indigo-800/80 pb-3">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-indigo-200">Vinh danh Top 5 học sinh có điểm số cao nhất (Nhấp vào học sinh để xem chi tiết bài làm)</p>
          </div>
        </div>
      ...
      <div className="space-y-2">
        {sortedSubmissions.map((sub, index) => {
          let rankBadge = null;
          let bgClass = 'bg-slate-800/60 border-slate-700 hover:bg-slate-800';

          if (index === 0) {
            rankBadge = <span className="text-xl">🥇</span>;
            bgClass = 'bg-gradient-to-r from-amber-500/20 to-yellow-600/30 border-amber-400/60 ring-2 ring-amber-400/30 hover:border-amber-300';
          } else if (index === 1) {
            rankBadge = <span className="text-xl">🥈</span>;
            bgClass = 'bg-gradient-to-r from-slate-300/20 to-slate-400/20 border-slate-300/50 hover:border-slate-200';
          } else if (index === 2) {
            rankBadge = <span className="text-xl">🥉</span>;
            bgClass = 'bg-gradient-to-r from-amber-700/20 to-amber-800/20 border-amber-600/50 hover:border-amber-400';
          } else {
          ...
          return (
            <div
              ...
              className={`p-3 rounded-2xl border flex items-center justify-between transition cursor-pointer shadow-2xs hover:scale-[1.005] active:scale-[0.995] ${bgClass}`}
              title="👉 Bấm vào đây để xem chi tiết bài làm và các câu sai của học sinh này"
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h5 className="font-extrabold text-xs text-white flex items-center space-x-1.5">
                    ...
                    <span className="text-[10px] text-amber-300 font-normal underline ml-1">(Bấm xem bài làm)</span>
                  </h5>
