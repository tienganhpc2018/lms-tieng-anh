...
export function exportClassExcelReport(submissions = [], activityTitle = 'Báo Cáo Bài Thi') {
  if (!submissions || submissions.length === 0) {
    alert('Không có dữ liệu bài nộp để xuất Báo cáo Excel!');
    return;
  }

  let csvContent = 'data:text/csv;charset=utf-8,\uFEFF';
  csvContent += 'STT,Họ và Tên Học Sinh,Điểm Số,Số Câu Đúng,Tổng Số Câu,Thời Gian Nộp,Trạng Thái\n';

  submissions.forEach((sub, idx) => {
    const sName = (sub.profiles?.full_name || sub.student_name || sub.answers_data?.student_name || sub.answers?.student_name || 'Học Sinh').replace(/,/g, ' ');
    const score = sub.score !== null && sub.score !== undefined ? sub.score : (sub.answers_data?.results?.scorePercent || 0);
    const correctCount = sub.correct_count !== undefined ? sub.correct_count : (sub.answers_data?.results?.correctGaps || 0);
    const totalQ = sub.total_questions !== undefined ? sub.total_questions : (sub.answers_data?.results?.totalGaps || 0);
    const timeStr = sub.submitted_at ? new Date(sub.submitted_at).toLocaleString('vi-VN').replace(/,/g, ' ') : 'N/A';
    const status = sub.status || 'Đã nộp';

    csvContent += `${idx + 1},${sName},${score},${correctCount},${totalQ},${timeStr},${status}\n`;
  });

  const encodedUri = encodeURI(csvContent);
  const link = document.createElement('a');
  link.setAttribute('href', encodedUri);
  link.setAttribute('download', `Bang_Diem_${(activityTitle || 'Bai_Thi').replace(/\s+/g, '_')}_${Date.now()}.csv`);
